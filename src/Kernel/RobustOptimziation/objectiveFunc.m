...
function [F]= objectiveFunc(designInput,Inp,optCnd,srgModel,FirstEval,uncerState)
...
noiDesOfExp=optCnd.noiDesOfExp;
    ...
    switch stochMthd
        case {1,2} % Monte Carlo Analysis (1.Random or 2.LHS)
            for position = 1:size(outID,2)
                if skewObjCon(position)==1
                    [mu_x,sigma_x,skew_x,~] = MCUQ(designInput,srgModel,noiDesOfExp,position); % mean and standard deviation response
                ...
                elseif skewObjCon(position)==0
                    [mu_x,sigma_x] = MCUQ(designInput,srgModel,noiDesOfExp,position); % mean and standard deviation response
                    uncerState.mu(position).mu=mu_x;
                    uncerState.sigma(position).sigma=sigma_x;
                end
            end
        ...
        case 3 % Analytical evaluation
            for position = 1:size(outID,2)
                if skewObjCon(position)==1
                    [Noisestruct,y_analytic,stdev_analytic,~,~,~,skewness]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
                ...
                elseif skewObjCon(position)==0
                    [Noisestruct,y_analytic,stdev_analytic]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
                    uncerState.mu(position).mu=y_analytic;
                    uncerState.sigma(position).sigma=stdev_analytic;
                    uncerState.Noisestruct=Noisestruct;
                end
