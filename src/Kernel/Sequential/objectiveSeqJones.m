...
function [F]= objectiveSeqJones(designInput,Inp,OptCnd,srgModel,FirstEval,Fmin,omega,uncerState)
...
switch stochMthd
    ...
    case 3 % Analytical
        for position = 1:1  %Only on Objective function
            if skewObjIncl==1
                [Noisestruct,y_anal,std_anal,~,~,s_hat,Skewness]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
            ...
            elseif skewObjIncl==0
                [Noisestruct,y_anal,std_anal,~,~,s_hat]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
                uncerState.mu(position).mu=y_anal;
                uncerState.sigma(position).sigma=std_anal;
                uncerState.s_hat(position).s_hat=s_hat;
                uncerState.Noisestruct=Noisestruct;
