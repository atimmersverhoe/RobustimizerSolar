...
function [F]= objectiveSeqMaxS(designInput,Inp,OptCnd,srgModel,FirstEval,uncerState)
...
switch stochMthd
    case {1,2} % Monte Carlo Analysis (1.Random or 2.LHS)
        for position = 1:1 %Location response in MMID
            if skewObjIncl==1
                [mu_x,sigma_x,skew_x,s_hat] = MCUQ(designInput,srgModel,noiDesOfExp,position); % mean and standard deviation response
                ...
                [mu_x,sigma_x,~,s_hat] = MCUQ(designInput,srgModel,noiDesOfExp,position); % mean and standard deviation response
                uncerState.mu(position).mu=mu_x;
                uncerState.sigma(position).sigma=sigma_x;
                uncerState.s_hat(position)=s_hat;
            end
    ...
    case 3 % Analytical
        for position = 1:1  %Only on Objective function
            if skewObjIncl==1
                [Noisestruct,y_anal,std_anal,~,~,s_hatAnalytic,Skewness]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
            ...
            elseif skewObjIncl==0
                [Noisestruct,y_anal,std_anal,~,~,s_hatAnalytic]=analyticalUQ(designInput,Inp,srgModel,position,FirstEval,uncerState.Noisestruct,uncerState.Flag);
                uncerState.mu(position).mu=y_anal;
                uncerState.sigma(position).sigma=std_anal;
                uncerState.s_hat(position)=s_hatAnalytic;
                uncerState.Noisestruct=Noisestruct;
